...
SELECT 
    ...
    SUBSTR('SQL Developer', 1, 3), -- 문자열 자름 1번부터 3개자름 (첫글자가 1번 <-> 자바는 0번)
...
SELECT 
    ...
    REPLACE('Java Programmer Java', 'Java', 'BigData') AS "REPLACE" -- 문자를 변경 Java 를 BigData로
...
FROM dual;
-- '$999,999' 가 없으면 오류
-- 화폐단위임을 알려서 숫자로 변환할 때 $가 무시되게 해야한다
...
SELECT 
    -- MAX(emp_nm)
		-- 1. MAX() 안에 공집합이 조회되면 null로 바꿔주며,
		-- 2. NVL 과 함께 써서 null이 아니라 어떻게 표현할 지 정할 수 있다.
--     NVL(emp_nm, '존재안함') AS emp_nm
