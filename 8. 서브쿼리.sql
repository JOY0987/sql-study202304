...
WHERE dept_cd = (
					...
					FROM tb_emp 
					WHERE emp_nm = '이관심'
				) -- 이관심이 2명이기 때문에 오류가 발생
;
-- = 로 비교하려면 우항에 두 개 이상의 컬럼이 오면 안 됨!
...
-- # 다중행 서브쿼리
-- 서브쿼리의 조회 건수가 0건 이상인 것 (단일행이 나와도 다중행 서브쿼리 포함)
-- ## 다중행 연산자
...
ORDER BY A.emp_no
; 
-- ANY 는 부등호에서 자주 쓴다. = 로 쓰면 IN 과 같음
-- ALL 은 서브쿼리를 모두 만족하는 컬럼만 조회된다.
...
WHERE addr LIKE '%강남%'
;
-- WHERE 절에서 2명이 선택되었으므로 1이 2번 조회된다.
